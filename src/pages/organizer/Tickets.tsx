// src/pages/organizer/Tickets.tsx - FIXED: Full ticket number display
  ...
  const loadTickets = async () => {
    ...
    try {
      ...
      const formatted: Ticket[] = (data || []).map((t: any) => {
        // ✅ FIX: Use order_id as the ticket number (full, not truncated)
const ticketNum = t.reference || `T-${t.id.substring(0, 8).toUpperCase()}`;
  ...
  const generateTicketPDF = async (ticket: Ticket) => {
    try {
      ...
      // Use full ticket number (order_id) - may wrap, but PDF handles long strings
      doc.text(ticket.ticket_number, 70, yPos, { maxWidth: 120 });
  ...
  const TicketCard = ({ ticket }: { ticket: Ticket }) => (
    <div className="bg-gradient-to-br from-white/5 to-white/2 backdrop-blur-xl border border-white/10 rounded-2xl overflow-hidden hover:border-purple-500/50 transition-all group hover:shadow-xl hover:shadow-purple-500/10">
      <div className="p-4 sm:p-6">
        ...
        {/* Ticket Number Section - with break-all for long numbers */}
          ...
          <div className="flex flex-col gap-3">
            <div className="flex-1">
              ...
              <div className="flex items-center gap-3">
                <code className="text-base sm:text-lg font-mono font-bold text-white bg-black/30 px-2 sm:px-3 py-1 rounded-lg break-all">
                  {ticket.ticket_number}
                </code>
                <button
          ...
          <main className="p-4 sm:p-6 lg:p-8 max-w-7xl mx-auto">
                ...
                {[
                  ...
                  <div
                    ...
                    className={`bg-gradient-to-br from-${stat.color}-500/10 to-${
                      stat.color === "purple" ? "pink" :
                      stat.color === "blue" ? "cyan" :
                      stat.color === "green" ? "emerald" :
                      stat.color === "yellow" ? "orange" : stat.color
            ...
            {loading ? (
              ...
              <div className="text-center py-16 sm:py-20">
                ...
                {(searchTerm || filterStatus !== "all" || filterEvent !== "all" || filterType !== "all") && (
                  <button onClick={clearFilters} className="px-6 py-3 bg-purple-600 hover:bg-purple-700 rounded-xl text-white font-medium transition">
                    Clear Filters
                  </button>
                )}
              </div>
        ...
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4" onClick={() => setSelectedQR(null)}>
          <div className="bg-gray-900 border border-white/20 rounded-xl sm:rounded-2xl p-6 sm:p-8 max-w-sm w-full text-center" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="flex flex-col sm:flex-row gap-3">
              <button onClick={() => { downloadQR(selectedQR, "ticket"); setSelectedQR(null); }} className="flex-1 bg-purple-600 hover:bg-purple-700 px-4 sm:px-6 py-3 rounded-xl text-white font-medium transition">
                Download QR
              </button>
              <button onClick={() => setSelectedQR(null)} className="flex-1 bg-white/10 hover:bg-white/20 px-4 sm:px-6 py-3 rounded-xl text-white font-medium transition">
                Close
              </button>
            </div>
          </div>
        ...
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-2 sm:p-4">
          <div className="bg-gray-900 border border-white/20 rounded-xl sm:rounded-2xl p-4 sm:p-6 lg:p-8 max-w-2xl w-full max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
            <div className="flex justify-between items-start mb-4 sm:mb-6">
              ...
              <button onClick={() => setSelectedTicket(null)} className="p-1 sm:p-2 hover:bg-white/10 rounded-lg transition">
            ...
            <div className="space-y-4 sm:space-y-6">
              <div className="bg-white/5 rounded-xl p-4 sm:p-5">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
                  <div><p className="text-gray-400 text-xs sm:text-sm">Event Name</p><p className="text-white font-medium text-sm sm:text-base">{selectedTicket.event_title}</p></div>
                  {selectedTicket.event_date && <div><p className="text-gray-400 text-xs sm:text-sm">Event Date</p><p className="text-white font-medium text-sm sm:text-base">{formatDate(selectedTicket.event_date)}</p></div>}
                  {selectedTicket.event_venue && <div><p className="text-gray-400 text-xs sm:text-sm">Venue</p><p className="text-white font-medium text-sm sm:text-base">{selectedTicket.event_venue}</p></div>}
                  {selectedTicket.event_location && <div><p className="text-gray-400 text-xs sm:text-sm">Location</p><p className="text-white font-medium text-sm sm:text-base">{selectedTicket.event_location}</p></div>}
                </div>
              </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
                  <div>
                    ...
                    <div className="flex items-center gap-2">
                      <code className="text-white font-mono font-bold text-sm sm:text-base break-all">{selectedTicket.ticket_number}</code>
                      <button onClick={() => copyToClipboard(selectedTicket.ticket_number, "Ticket number")} className="p-1 hover:bg-white/10 rounded flex-shrink-0">
                        <Copy className="w-3 h-3 sm:w-4 sm:h-4 text-gray-400" />
                      </button>
                    </div>
                  </div>
                  <div><p className="text-gray-400 text-xs sm:text-sm">Ticket Type</p><p className="text-white font-medium text-sm sm:text-base">{selectedTicket.ticket_type}</p></div>
                  <div><p className="text-gray-400 text-xs sm:text-sm">Quantity</p><p className="text-white font-medium text-sm sm:text-base">{selectedTicket.quantity}</p></div>
                  <div>
                    <p className="text-gray-400 text-xs sm:text-sm">Status</p>
                    <div className="flex items-center gap-2">
                      <span className={`px-2 py-1 rounded text-xs font-medium ${selectedTicket.verified_at ? "bg-emerald-500/20 text-emerald-300" : "bg-yellow-500/20 text-yellow-300"}`}>
                        {selectedTicket.verified_at ? "VERIFIED" : "PENDING"}
                      </span>
                      {!selectedTicket.verified_at && (
                        <button onClick={() => verifyTicket(selectedTicket.id)} disabled={verifyingTicket === selectedTicket.id} className="px-2 py-1 bg-emerald-500/20 text-emerald-300 hover:bg-emerald-500/30 rounded text-xs disabled:opacity-50 whitespace-nowrap">
              ...
              <div className="bg-white/5 rounded-xl p-4 sm:p-5">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
                  <div><p className="text-gray-400 text-xs sm:text-sm">Full Name</p><p className="text-white font-medium text-sm sm:text-base">{selectedTicket.buyer_name}</p></div>
                  <div>
                    <p className="text-gray-400 text-xs sm:text-sm">Email</p>
                    <div className="flex items-center gap-2">
                      <p className="text-white font-medium text-sm sm:text-base truncate">{selectedTicket.buyer_email}</p>
                      <button onClick={() => copyToClipboard(selectedTicket.buyer_email, "Email")} className="p-1 hover:bg-white/10 rounded flex-shrink-0">
                        <Copy className="w-3 h-3 sm:w-4 sm:h-4 text-gray-400" />
                      </button>
                    </div>
                  </div>
              ...
              <div className="bg-white/5 rounded-xl p-4 sm:p-5">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
                  <div><p className="text-gray-400 text-xs sm:text-sm">Amount Paid</p><p className="text-white font-medium text-sm sm:text-base">{formatCurrency(selectedTicket.price)}</p></div>
                  <div>
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <button onClick={() => copyToClipboard(selectedTicket.order_id, "Order ID")} className="p-1 hover:bg-white/10 rounded flex-shrink-0">
                        <Copy className="w-3 h-3 sm:w-4 sm:h-4 text-gray-400" />
                      </button>
                    </div>
                  </div>
                  <div>
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <button onClick={() => copyToClipboard(selectedTicket.reference, "Reference")} className="p-1 hover:bg-white/10 rounded flex-shrink-0">
                        <Copy className="w-3 h-3 sm:w-4 sm:h-4 text-gray-400" />
                      </button>
                    </div>
                  </div>
                  <div><p className="text-gray-400 text-xs sm:text-sm">Purchase Date</p><p className="text-white font-medium text-sm sm:text-base">{formatDate(selectedTicket.purchased_at)}</p></div>
                  {selectedTicket.verified_at && <div><p className="text-gray-400 text-xs sm:text-sm">Verification Date</p><p className="text-white font-medium text-sm sm:text-base">{formatDate(selectedTicket.verified_at)}</p></div>}
                </div>
              </div>
              ...
              <div className="flex flex-wrap gap-2 sm:gap-3 pt-4 border-t border-white/10">
                {selectedTicket.qr_code_url && (
                  <>
                    <button onClick={() => setSelectedQR(selectedTicket.qr_code_url)} className="flex items-center gap-2 px-3 sm:px-4 py-2 bg-purple-600 hover:bg-purple-700 rounded-xl text-white text-sm font-medium transition flex-1 sm:flex-none justify-center">
                      <QrCode className="w-4 h-4" /> View QR
                    </button>
                    <button onClick={() => downloadQR(selectedTicket.qr_code_url!, `${selectedTicket.ticket_number}-QR`)} className="flex items-center gap-2 px-3 sm:px-4 py-2 bg-white/10 hover:bg-white/20 rounded-xl text-white text-sm font-medium transition flex-1 sm:flex-none justify-center">
                      <Download className="w-4 h-4" /> Download QR
                    </button>
                  </>
                )}
                <button onClick={() => generateTicketPDF(selectedTicket)} className="flex items-center gap-2 px-3 sm:px-4 py-2 bg-blue-600/20 hover:bg-blue-600/30 border border-blue-500/30 rounded-xl text-blue-300 text-sm font-medium transition flex-1 sm:flex-none justify-center">
                  <FileText className="w-4 h-4" /> Generate PDF
                </button>
                {!selectedTicket.verified_at && (
                  <button onClick={() => verifyTicket(selectedTicket.id)} disabled={verifyingTicket === selectedTicket.id} className="flex items-center gap-2 px-3 sm:px-4 py-2 bg-emerald-600 hover:bg-emerald-700 rounded-xl text-white text-sm font-medium transition flex-1 sm:flex-none justify-center disabled:opacity-50">
                    <CheckCircle className="w-4 h-4" />
