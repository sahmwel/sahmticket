...
export default function Sidebar({ role, isMobileOpen, onMobileClose }: SidebarProps) {
  ...
  return (
    <>
      ...
      {/* Sidebar */}
      <aside className={`
        fixed left-0 top-0 h-full w-64 bg-gradient-to-b from-gray-900 to-gray-950 text-white border-r border-gray-800 flex flex-col z-50
        transform transition-transform duration-300 md:translate-x-0
        ${isMobileOpen ? 'translate-x-0' : '-translate-x-full md:translate-x-0'}
      `}>
        {/* Logo & Header with Organizer Panel Label */}
              ...
              <div key={index} className="space-y-2">
                ...
                <div className={`space-y-1 ${activeGroup === group.title ? 'block' : 'hidden'}`}>
                  {group.links.map((link) => (
                    ...
                    >
                      <div className={`${
                        link.path.includes('/organizer/create-event') || link.path.includes('/admin/analytics') 
                          ? "text-purple-400" 
        ...
        <div className="p-4 border-t border-gray-800">
          <button 
            className="flex items-center gap-3 w-full px-3 py-3 text-gray-400 hover:text-red-400 hover:bg-red-500/10 rounded-lg transition-all group"
            onClick={async () => {
