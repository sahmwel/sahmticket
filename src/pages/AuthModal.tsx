...
export default function AuthModal({ onClose }: AuthModalProps) {
  ...
  const handleSignUp = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const { data, error } = await supabase.auth.signUp({
        email,
        password,
        options: { emailRedirectTo: window.location.origin }, // no email confirmation
      });
  ...
  const handleForgotPassword = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    setLoading(true);
    setMessage(null);
    if (!forgotEmail) return setMessage("Email is required.");

    try {
      // Delegate password reset to your backend
      await fetch("/functions/v1/send-reset-link", {
        method: "POST",
